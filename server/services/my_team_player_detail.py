...
matches.round_detail_json/match_player_stats만으로 계산한다. player_stats_summary가
채워지지 않은 선수(=이 페이지 첫 방문)에 한해 계산 후 캐싱하는 cache-aside이고, 이후
조회는 캐시만 읽는다. 나중에 "갱신" 버튼이 생기면 그때 재계산 트리거를 추가하면 됨 -
지금은 신경쓰지 않는다.
...
이 페이지에서만 새로 필요했던 정의:
  ...
  - 트레이드 1v1/1v2: 상대 인원수가 아니라 "내가 죽은 교전에서 상대가 몇 명 같이
    죽었는지"로 정의한다. round_phase_analysis.py::nearby_trade_deaths 공유 함수로
    계산 - my_team_analysis.py(팀 분석)의 트레이드 성공률도 같은 함수를 쓴다. 1v1 =
    같은 교전에서 상대가 1명 이상 같이 죽음(기본 트레이드), 1v2 = 2명 이상(가치
    트레이드) - 두 비율은 서로 배타적이지 않고 각각 전체 사망 횟수 대비 비율이다(1v2
    상황은 1v1 상황에도 포함됨).
  - 스킬 사용 유효율: Henrik의 ability_casts가 항상 null이라 데이터 소스가 없어 항상
    빈 배열로 둔다.
