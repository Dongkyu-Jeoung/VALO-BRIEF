...
# ============================================================================
# 죽은 코드(사용되지 않음) - 2026-09-15 주석 처리.
#
# 아래 세 함수(_backfill_if_needed, _insert_match, _sync)는 이 파일 하단에 있는
# 두 번째 _sync(및 sync_team_match_history가 실제로 호출하는 버전)와 이름이 겹쳐서
# 파이썬이 나중 정의로 덮어써 버렸다 - 즉 지금은 아무 곳에서도 호출되지 않는다.
# 파일 최상단 docstring("Raw matches and player statistics are no longer inserted
# or updated here")대로 원본 매치/선수 스탯 저장 로직이 services/match_history.py로
# 이관되면서 남은 옛날 버전으로 보인다.
#
# 이 블록이 참조하는 Match, MatchPlayerStat, RiotAccount, ROLE_LABELS,
# engagement_predictor, _find_registered_team, _parse_game_start, _now_kst,
# _parse_started_at, upsert_riot_account, _ensure_riot_account_placeholder,
# _load_agent_meta_by_name, _load_weapon_uuids는 이 파일에서 더 이상 import/정의되지
# 않아 Pylance가 "is not defined"로 표시한다(Problems 패널 26건의 원인).
#
# 실제로 죽은 코드가 맞는지 재확인한 뒤 삭제해도 안전하지만, 우선은 삭제 대신
# 주석 처리만 해둔다. 실수로 필요했던 로직이면 아래에서 그대로 복구할 수 있다.
# ============================================================================
